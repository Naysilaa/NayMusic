            ...
            [   
                ...
                [
                    InlineKeyboardButton(
                        ...
                        url=f"https://www.instagram.com/antoniprananda")
            ]        
        ]
    )
    return mar
...
async def play(_, message: Message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    await lel.edit("📣Pencarian Ditemukan...")
...
async def deezer(client: Client, message_: Message):
    ...
    keyboard = InlineKeyboardMarkup(
         [   
             [
                 InlineKeyboardButton('Playlist', callback_data='playlist'),
                 InlineKeyboardButton('Setting', callback_data='menu')     
             ],                     
             [
                 InlineKeyboardButton(
                     text="Official Group",
                     url="https://t.me/VcgSupportGroup")
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"**Rindu**= #️⃣ Queued at position {position}")
    else:
